...
#define UART_COM_REG             UCSRC
#define UART_REG_SELECT_BIT      URSEL
#define UART_CHAR_SIZE_0         UCSZ0
#define UART_CHAR_SIZE_1         UCSZ1
...
{
    /*
     * Initialize timer1, will be used for timeout.
     */
    TCCR1B |= _BV(CS12) | _BV(CS10); // Prescaler CLK/1024
    TCNT1 = 0; // Reset Timer1 counter
    while(!(UART_STATUS_REG & (1 << UART_RECEIVE_COMPLETE))) {
        if (TCNT1 > MAX_TIME_COUNT) {
            TCCR1B = 0;
            app_start();
        }
    }
...
/*
 * Converts the lower nibble of a char to hexadecimal value.
 */
...
void otherMessage(const char *message) {
    static char id[ID_SIZE*2+1]="";
    char ch;
    uint8_t i;
    uint8_t crc=0x11;
    if(id[0] == 0) {
        for(i=0;i<ID_SIZE*2;i+=2) {
            ch = eeprom_read_byte((uint8_t *)(i>1));
            if(ch!=0xFF) {
                id[i] = to_hex(ch)>>4;
                id[i+1] = to_hex(ch);
            } else {
    ...
    sendchar(0x02); // Start Text message (ascii STX)
    sendchar(0x0);  // Message length (16 bit, high)
    sendchar(0x11);  // Message length (16 bit, low)
...
#ifdef UART_COM_REG
        UART_COM_REG = (1 << UART_REG_SELECT_BIT) | (1 << UART_CHAR_SIZE_0) | (1 << UART_CHAR_SIZE_1);
#endif


    /*
     * We're the happy bootloader, start spamming the uart so the other side
     * knows it's us!
     */
    otherMessage("OLA!");
